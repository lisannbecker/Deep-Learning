...
class DeepPromptCLIP(nn.Module):
    ...
    def __init__(self, args, dataset, template="This is a photo of {}"):
        ...
        size_dp = (50, args.batch_size, 768) #128 - args.batch_size?

        for i in range(args.prompt_num - 1):
            if args.device == 'cpu':
                self.deep_prompt = nn.Parameter(torch.randn(size_dp, dtype=torch.float32))
            elif args.device == 'gpu':
                self.deep_prompt = nn.Parameter(torch.randn(size_dp, dtype=torch.float16))
    ...
    def forward(self, image):
        ...
        img_features = self.custom_encode_image(image)
    ...
    def custom_encode_image(self, x): #x is the image
        ...
        # - Inject the deep prompt at the specified layer (self.injection_layer).
            if i == self.injection_layer:
                ...
                # Add the deep prompt to the input before the transformer block.
